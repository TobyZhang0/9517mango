...
def get_transforms(augmentation_strategy='default'):
    if augmentation_strategy == 'minimal':
        train_transform = transforms.Compose([
            ...
            transforms.ToTensor(),  # 确保在此处转换为 Tensor
        ...
        train_transform = transforms.Compose([
            ...
            transforms.ToTensor(),  # 确保在此处转换为 Tensor
            transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
            transforms.RandomErasing(p=0.2)  # RandomErasing 应该在 ToTensor 之后
        ])
        ...
        train_transform = transforms.Compose([
            ...
            transforms.RandomEqualize(),  # RandomErasing 应该在 ToTensor 之后
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        ])
    else:
